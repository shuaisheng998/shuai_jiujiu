  ...
  static List<MathQuestion> getGrade7Questions() {
    return [
      ...
      MathQuestion(
        ...
        steps: [
          '📌 解题策略：等腰三角形问题 → 先回忆两个关键性质，再用内角和定理',
          '',
          '🧠 前置知识复习（可能会忘，先回顾）：',
          '  ① 三角形内角和定理：任意三角形的三个内角之和 = 180°',
          '     （这是平面几何最基础的定理，所有三角形都适用）',
          '  ② 等腰三角形定义：有两条边相等的三角形',
          '     性质1：两腰相等（定义本身）',
          '     性质2：两底角相等（等边对等角，证明可用全等三角形SSS）',
          '  ③ 什么是"底角"？等腰三角形中，相等的两条边叫"腰"，',
          '     第三条边叫"底边"，底边两端的角叫"底角"，腰对面的角叫"顶角"',
          '',
          '第1步：已知条件整理',
          '  顶角 = 80°，两底角相等（设为x），三角形内角和 = 180°',
          '',
          '第2步：用内角和定理列方程',
          '  顶角 + 底角1 + 底角2 = 180°',
          '  80° + x + x = 180°',
          ...
          '✅ 验证：50° + 50° + 80° = 180° ✓',
          '',
          '⚠️ 常见错误：180 - 80 = 100是"两个底角的和"，不是"一个底角"！',
          '   一定要记得 ÷2，不然会错选100°',
          '',
          '💡 方法总结：等腰三角形求角的通用公式—底角 = (180° - 顶角) ÷ 2',
          '   如果已知底角求顶角：顶角 = 180° - 2×底角',
        ],
      ...
      MathQuestion(
        ...
        steps: [
          '📌 解题策略：三角形面积 → 回忆公式 S = ½ × 底 × 高',
          '',
          '🧠 前置知识复习（这里的"高"很多人搞不清）：',
          '  ① 长方形面积 = 长 × 宽（这是小学知识，但所有面积公式的基础）',
          '  ② "高"的定义：从三角形一个顶点向对边（或对边延长线）作垂线，',
          '     顶点到垂足的距离就是"高"。注意：高不一定在三角形内部！',
          '     钝角三角形的高可能在三角形外面！',
          '  ③ "垂线"即两条线相交成90°（直角），也就是"线线垂直"',
          '     判断垂直：两线夹角 = 90°',
          '  ④ 为什么三角形面积是底×高÷2？',
          '     把一个平行四边形沿对角线切成两半，得到两个一样的三角形',
          '     平行四边形面积 = 底×高 → 三角形 = 平行四边形÷2 = 底×高÷2',
          '',
          '第1步：长方形面积 = 长 × 宽（最基础的面积公式）',
          '第2步：三角形可以看作是长方形的一半',
          '   → 画一个长方形，沿对角线剪开 → 两个全等三角形',
          '第3步：三角形面积 = 长方形面积 ÷ 2 = (底 × 高) ÷ 2',
          '',
          '⚠️ 常见错误：底和高必须对应！',
          '  选哪条边做"底"，就要用"这条底边上的高"',
          '  不能用A边做底、B边的高来计算！',
      ...
      MathQuestion(
        ...
        steps: [
          '📌 解题策略：看到直角三角形+两条边求第三边 → 勾股定理！',
          '',
          '🧠 前置知识复习（这些定理你可能学过但容易忘）：',
          '  ① 什么是直角三角形？有一个角 = 90°（直角）的三角形',
          '     → 构成直角的两条边叫"直角边"（较短的两条）',
          '     → 直角对面的边叫"斜边"（永远是最长的那条边）',
          '  ② 勾股定理：直角边² + 直角边² = 斜边²（a² + b² = c²）',
          '     历史：中国古代叫"勾三股四弦五"，西方叫毕达哥拉斯定理',
          '  ③ 为什么这个定理成立？可以这样理解：',
          '     以三条边为边长各画一个正方形，两个小正方形的面积和 = 大正方形面积',
          '  ④ 平方运算：a² = a × a（自己乘自己）',
          '  ⑤ 开方运算：√25 = 5（因为5²=25），平方和开方互为逆运算',
          '',
          '第1步：确认已知条件',
          '  直角边 a = 3，直角边 b = 4，求斜边 c',
          '  （注意：c必须是斜边，不能搞反！斜边是直角的对边）',
          '',
          '第2步：代入勾股定理 a² + b² = c²',
          '  3² + 4² = c²',
          '',
          ...
          '  因此斜边 c = 5',
          '',
          '✅ 验证：3²+4²=9+16=25, 5²=25，等式成立 ✓',
          '',
          '⚠️ 易错提醒：',
          '  • 直角边和斜边不能搞混！斜边一定是最长的那条',
          '  • 如果题目给的是斜边+一条直角边，求另一条直角边，',
          '    公式变成：直角边² = 斜边² - 另一直角边²',
          '  • 不是直角三角形不能用勾股定理！',
          '',
          '💡 常用勾股数（背下来，秒杀选择填空）：',
      ...
      MathQuestion(
        ...
        steps: [
          '📌 解题策略：正负数加法 → 先判断同号还是异号 → 选规则 → 计算',
          '',
          '🧠 前置知识复习（正负数运算规则，必须烂熟于心）：',
          '  ① 什么是"同号"和"异号"？',
          '     同号：两个数符号相同（都正 或 都负）',
          '     异号：符号不同（一正一负）',
          '  ② 什么是"绝对值"？',
          '     绝对值 = 一个数到0的距离（永远是正数或0）',
          '     符号 |x| 表示，如 |-12| = 12，|8| = 8',
          '  ③ 加法规则（牢记）：',
          '     ★ 同号相加：符号不变，绝对值相加',
          '        例：(-3)+(-5) = -(3+5) = -8',
          '     ★ 异号相加：取绝对值大的那个数的符号，用大绝对值减小绝对值',
          '        例：(-12)+8 → |-12|=12 > |8|=8 → 取负号 → 12-8=4 → 结果=-4',
          '  ④ 生活类比：赚钱是正数(+)，欠钱是负数(-)',
          '     欠12块(-12) + 赚8块(+8) = 还欠4块(-4)',
          '',
          '第1步：判断符号：-12（负）和 +8（正）→ 异号',
          '第2步：异号用规则：取绝对值大的符号',
          '  |-12| = 12，|8| = 8，12 > 8 → 取负号',
          '第3步：大绝对值减小绝对值：12 - 8 = 4',
          '第4步：加上符号：-4',
          '',
          '💡 口诀：同号相加不变号，绝对值加；异号相加看大小，大减小取大号',
        ],
      ...
      MathQuestion(
        ...
        steps: [
          ...
          '     相反数：符号相反、绝对值相同的两个数',
          '     如：3的相反数是-3，-5的相反数是5',
          '  ② 关键规则：减去一个负数 = 加上它的相反数（正数）',
          '     (-7) - (-3) = (-7) + (+3) = (-7) + 3',
          '     这被称为"减负得正"',
          '  ③ 为什么"减负得正"？直观理解：',
          '     "减去欠的债" = "获得钱"，减少亏损就是增加',
          '     欠7块(-7)，减掉3块的债(-(-3)) = 加3块钱(+3) = 还欠4块(-4)',
          '',
          '第1步：(-7) - (-3)，减负数，变成加正数',
          '  (-7) - (-3) = (-7) + 3',
          '',
      ...
      MathQuestion(
        ...
        explanation: '两边加5',
        steps: [
          '📌 解题策略：解方程的目标 → 把x单独留在等式左边，数字移到右边',
          '',
          '🧠 前置知识复习（解方程的基石——等式性质）：',
          '  ① 什么是方程？含有未知数(字母)的等式叫方程',
          '     "解方程"就是找到让等式成立的未知数的值',
          '  ② 等式性质1（加減原理）：',
          '     等式两边同加或同減一个相同的数，等式仍然成立',
          '     例如：如果 a = b，那么 a + 3 = b + 3',
          '  ③ 等式性质2（乘除原理）：',
          '     等式两边同乘或同除一个不为0的数，等式仍然成立',
          '     例如：如果 a = b，那么 a × 2 = b × 2',
          '  ④ "移项"的本质：把一项从等号一边移到另一边，符号要变',
          '     x - 5 = 12 中的 -5 移到右边 → 变成 +5',
          '     理解：其实是在两边同时加5！',
          '  ⑤ 解是方程的解吗？一定要代回检验！',
          '',
          '第1步：原方程 x - 5 = 12',
          ...
          '',
          '第2步：利用等式性质1，两边同时加5',
          '  为什么加5？因为左边 -5+5=0，可以消掉-5',
          '  左边：x - 5 + 5 = x',
          '  右边：12 + 5 = 17',
          '',
          '第3步：所以 x = 17',
          '',
          '✅ 检验：17 - 5 = 12 ✓ 等式成立，答案正确！',
          '',
          '💡 简单记忆："移项要变号" —— -5 移过去变成 +5',
        ],
      ...
      MathQuestion(
        ...
        steps: [
          '📌 解题策略：有括号的方程 → 两种思路：先展开 或 先两边除以括号前的数',
          '',
          '🧠 前置知识复习：',
          '  ① 乘法分配律：a(b + c) = ab + ac',
          '     a(b - c) = ab - ac',
          '     这是代数最基础的法则，几乎所有方程都会用到',
          '  ② 解法一（展开法）：先去括号，再移项',
          '     3(x - 2) = 9 → 3x - 6 = 9 → 3x = 15 → x = 5',
          '  ③ 解法二（简便法）：两边先除以3',
          '     3(x - 2) ÷ 3 = 9 ÷ 3 → x - 2 = 3 → x = 5',
          '     推荐！因为右边9能被3整除',
          '',
          '第1步：观察到右边9刚好是3的倍数，用解法二更快',
          '  3(x - 2) = 9',
          '  两边同时 ÷ 3（等式性质2）',
          '',
          '第2步：x - 2 = 3',
          ...
          '✅ 检验：3×(5-2) = 3×3 = 9 ✓',
          '',
          '💡 技巧：括号前有系数时，先看右边能不能被系数整除',
          '   能整除 → 先除再解（更快），不能整除 → 先展开',
        ],
      ...
      MathQuestion(
        ...
        steps: [
          '📌 解题策略：方程两边都有x → 先把含x的项移到一边，常数移到另一边',
          '',
          '🧠 前置知识复习（移项的核心规则）：',
          '  ① 移项 = 把等式一边的项移到另一边，同时变号',
          '     +2x 移到右边 → -2x',
          '     -1 移到右边 → +1',
          '  ② 为什么移项要变号？本质是两边同时加减同一个数',
          '     如：2x-1 = 5x+8',
          '     两边同时 -5x：2x-5x-1 = 8',
          '     两边同时 +1：2x-5x = 8+1',
          '     可见移项只是"两边同加减"的快捷写法',
          ...
          '  目标：把所有x移到左边，常数移到右边',
          '',
          '第2步：把右边的5x移到左边（变号：+5x→-5x）',
          '  2x - 5x - 1 = 8',
          '',
          '第3步：把左边的-1移到右边（变号：-1→+1）',
          '  2x - 5x = 8 + 1',
          '',
          '第4步：合并同类项',
          '  左边：2x - 5x = -3x',
      ...
      MathQuestion(
        ...
        explanation: '除以负数要变号',
        steps: [
          '📌 解题策略：不等式系数为负 → 两边同除时 → 不等号方向要翻转！',
          '',
          '🧠 前置知识复习（不等式性质——和等式不一样的地方）：',
          '  ① 不等式性质1（加减不变）：',
          '     两边同加/减同一个数，不等号方向不变',
          '     例：x + 3 > 5 → x > 2（两边都 -3，> 还是 >）',
          '  ② 不等式性质2（乘除正数不变）：',
          '     两边同乘/除一个正数，不等号方向不变',
          '     例：2x > 6 → x > 3（两边都 ÷2，> 还是 >）',
          '  ③ ★不等式性质3（乘除负数要变号！）：',
          '     两边同乘/除一个负数，不等号方向必须反过来！',
          '     例：-2x ≤ 6 → 两边都 ÷(-2) → x ≥ -3',
          '     ≤ 变成了 ≥！这就是"变号"的意思！',
          '  ④ 为什么除以负数要变号？直观理解：',
          '     5 > 3（正确），但如果两边都 ×(-1)：',
          ...
          '  6 ÷ (-2) = -3',
          '',
          '第3步：⚠️关键！因为除的是负数(-2)，不等号要变方向！',
          '  原来是 ≤（小于等于），现在变成 ≥（大于等于）',
          '  所以：x ≥ -3',
          '',
          '✅ 验证：取 x=0（满足 ≥-3），代入 -2×0=0 ≤ 6 ✓',
          '  取 x=-4（不满足 ≥-3），代入 -2×(-4)=8 > 6，不满足原不等式 ✓',
          '',
          '💡 口诀：不等式乘除负数，不等号要翻身！',
          '   这是不等式和方程最大的区别！方程乘除负数不用变号',
        ],
      ...
      // === 新增：有理数混合运算 ===
      MathQuestion(question: '计算：(-2)³ + 4 × (-3) = ?', options: ['-20', '-4', '4', '20'], correctIndex: 0, explanation: '先乘方再乘除后加减', steps: ['📌 解题策略：混合运算 → 牢记运算顺序：乘方→乘除→加减', '', '🧠 前置知识：①(-2)³=(-2)×(-2)×(-2)=-8 ②乘法：正×负=负 ③加减规则', '', '第1步：(-2)³ = -8（三个负数相乘得负）', '第2步：4×(-3) = -12（异号相乘得负）', '第3步：-8 + (-12) = -20', '', '⚠️ 易错：先算4×(-3)=-12，再把-8和-12相加，不是相减！'], knowledgePoint: '有理数混合运算：乘方>乘除>加减', category: '代数', level: 'grade7', difficulty: 3),
      MathQuestion(question: '若|x|=5，则x等于？', options: ['5', '-5', '5或-5', '0'], correctIndex: 2, explanation: '绝对值等于5的数有两个：5和-5', steps: ['📌 解题策略：|x|=a → x=±a（有两个解！）', '', '🧠 前置知识：①绝对值=|到0的距离| ②距离不能是负数 ③+5到0距离=5，-5到0距离也是5', '', '第1步：|x|=5 意思是x到0的距离=5', '第2步：在数轴上，距离0为5的点有两个：右边5和左边-5', '第3步：所以x可以=5，也可以=-5', '', '💡 规律：|x|=a（a>0）的解永远是 x=a 或 x=-a'], knowledgePoint: '绝对值方程：|x|=a的解为x=±a', category: '代数', level: 'grade7', difficulty: 2),
      MathQuestion(question: '计算：(-1/2) × 4 = ?', options: ['-2', '-1/2', '2', '1/2'], correctIndex: 0, explanation: '分数乘整数：分子乘整数，分母不变', steps: ['📌 分数×整数 = (分子×整数)/分母', '', '第1步：(-1/2)×4 = -(1×4)/2', '第2步：1×4=4，4/2=2', '第3步：因为是负数×正数，结果为负，=-2'], knowledgePoint: '分数乘法：(-a/b)×c = -(a×c)/b', category: '代数', level: 'grade7', difficulty: 2),
	      MathQuestion(question: '比较大小：-2/3 ___ -3/4', options: ['>', '<', '=', '不确定'], correctIndex: 0, explanation: '两个负数比较，绝对值大的反而小', steps: ['📌 负数比较规则：绝对值越大的负数越小', '', '第1步：|-2/3|=2/3≈0.667', '第2步：|-3/4|=0.75', '第3步：0.75>0.667，即|-3/4|>|-2/3|', '第4步：绝对值大的负数反而小，所以-2/3 > -3/4', '', '💡 数轴上：-2/3=-0.667 在右边，-3/4=-0.75 在左边，右边>左边'], knowledgePoint: '负数大小比较：绝对值大的反而小', category: '代数', level: 'grade7', difficulty: 2),
    ];
  }
