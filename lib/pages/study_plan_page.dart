...
class _StudyPlanPageState extends State<StudyPlanPage> {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      _wordGoal = wordGoal;
      _questionGoal = questionGoal;
      _wordsDone = wordsDone;
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) return Scaffold(appBar: AppBar(title: const Text('每日计划'), backgroundColor: theme.colorScheme.primary, foregroundColor: Colors.white), body: const Center(child: CircularProgressIndicator()));

    final wordPct = _wordGoal > 0 ? (_wordsDone / _wordGoal).clamp(0.0, 1.0) : 0.0;
    final qPct = _questionGoal > 0 ? (_questionsDone / _questionGoal).clamp(0.0, 1.0) : 0.0;
    final totalPct = (_wordGoal + _questionGoal > 0) ? (_wordsDone + _questionsDone) / (_wordGoal + _questionGoal) : 0.0;
    String msg;
    IconData icon;
    Color color;
    if (totalPct >= 1.0) { msg = '🎉 今日目标全部完成！太棒了！'; icon = Icons.emoji_events; color = Colors.amber; }
    else if (totalPct >= 0.6) { msg = '💪 快完成了，再加把劲！'; icon = Icons.trending_up; color = Colors.green; }
    else if (totalPct > 0) { msg = '🚀 好的开始，继续加油！'; icon = Icons.play_circle; color = Colors.blue; }
    else { msg = '📋 设定今日目标，开始学习吧！'; icon = Icons.edit_calendar; color = Colors.grey; }

    return Scaffold(
      appBar: AppBar(title: const Text('每日计划'), backgroundColor: theme.colorScheme.primary, foregroundColor: Colors.white),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
          ...
          const SizedBox(height: 14),
          _progressRow('背单词', _wordsDone, _wordGoal, wordPct, theme.colorScheme.primary),
          const SizedBox(height: 10),
          _progressRow('做题目', _questionsDone, _questionGoal, qPct, Colors.green),
          const SizedBox(height: 14),
          ClipRRect(borderRadius: BorderRadius.circular(6), child: LinearProgressIndicator(value: totalPct, minHeight: 12, backgroundColor: Colors.grey[200], color: totalPct >= 1 ? Colors.amber : theme.colorScheme.primary)),
